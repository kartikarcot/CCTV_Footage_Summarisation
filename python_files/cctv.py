...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow, num_items):
    ...
    def set_combobox(self):
        ...
        for filename in db_filenames:
            self.comboBox.setItemText(j, _translate("MainWindow", filename, None))
            j += 1

        # print(self.comboBox.currentIndex())

        self.comboBox.activated.connect(self.cal_length)
            # print(self.comboBox.currentIndex())
    ...
    def cal_length(self):
        ...
        db.create_connection()
        frame_count = db.get_clip_length(self.filename)
        self.duration = frame_count/30
        dur = self.duration
        # print(duration)
        self.hour = dur / 3600
        dur = dur % 3600
        ...
        dur = dur % 60
        self.sec = dur
        time = "{}hr {}min {}sec".format(int(self.hour),int(self.minute),int(self.sec))
        print(time)
        self.textBrowser.setText(time)

        #start time
        self.lineEdit.setText("0")
        m1 = self.lineEdit_3.setText("0")
        sec1 = self.lineEdit_4.setText("0")

        #end time
        h2 = self.lineEdit_2.setText(str(int(self.hour)))
        m2 = self.lineEdit_5.setText(str(int(self.minute)))
        sec2 = self.lineEdit_6.setText(str(int(self.sec)))
    ...
    def press_generate(self):
        ...
        print(tags)
        if len(tags) > 0:
            qry.generate_summary_by_query(filename=self.filename,tags=tags,start_frame = start_in_sec * 30, end_frame=end_in_sec * 30)
        else:
            qry.generate_summary_by_query(filename=self.filename,start_frame = start_in_sec * 30, end_frame=end_in_sec * 30)
...
if __name__ == "__main__":
    ...
    db.create_connection()
    num_clips = db.get_num_clips()
    ui.setupUi(MainWindow, num_clips)
    MainWindow.show()
    ui.set_combobox()
    ui.cal_length()
